...
export default function CinematicMode({ game, onSkip }: Props) {
  ...
  const { loaded, imgFailed } = useImagePreload(urlChain);
  const allExhausted = !urlChain.length || imgFailed >= urlChain.length;

  useEffect(() => {
    if (loaded) {
      const t1 = setTimeout(() => setMetaVisible(true), 300);
  ...
  return (
    <div className="absolute inset-0 vignette film-grain overflow-hidden bg-black">
      <AnimatePresence>
        {loaded && !allExhausted && imgSrc && (
          <motion.div key={imgFailed} className="absolute inset-0" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 1.8, ease: 'easeInOut' }}>
            <img src={imgSrc} alt="" className={`absolute inset-0 w-full h-full ${kbClass}`} style={{ objectFit: 'cover', filter: 'blur(30px) brightness(0.9)', willChange: 'filter' }} />
            <img src={imgSrc} alt={game.name} className="absolute inset-0 w-full h-full" style={{ objectFit: 'contain' }} onError={() => {}} />
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {metaVisible && (
          <motion.div className="absolute bottom-0 left-0 right-0 z-10 p-16" style={{ background: 'linear-gradient(transparent, rgba(0,0,0,0.7))' }} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.8, ease: 'easeOut' }}>
